...
Run the normal Godot editor import, then `BuildOvalScene.gd` as above. It invokes `BuildOvalContent.gd` to bake the fixed outer collision, concrete strip, exterior ground, MultiMesh forest batches and pickup markers into the scriptless production scene. The road geometry and road collision remain unchanged. The finite physical perimeter is baked by `BuildPerimeter.gd`; visible concrete and curved catch-mesh collision share geometry. Its original welded-mesh shader is `CatchFence.gdshader`. Regenerate with the full scene baker. The reference is TS-174 attachment 10084, `Concrete barrier.png`; it guides the low concrete wall and inward-curved steel mesh concept only, and no photograph pixels are redistributed.  Asphalt and concrete reuse the repository's existing licensed Poly Haven maps. `environment-sources.json` records original-art provenance and checksums. Preserve grass import settings for mipmaps/VRAM compression.
