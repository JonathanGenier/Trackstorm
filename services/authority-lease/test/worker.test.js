...
test("actual Worker HTTP and SQLite Durable Objects isolate sessions and serialize competing takeovers", async () => {
  ...
  try {
    ...
    assert.deepEqual(Object.keys(first).sort(), ["epoch", "holder", "remainingSeconds", "routingId", "session", "token"]);
    assert.match(first.routingId, /^[a-f0-9]{64}$/);
    assert.notEqual(first.routingId, session);
    const routeRequest = { session: first.routingId, epoch: 0, token: "" };
    assert.equal((await send("route", routeRequest, null)).status, 401);
    assert.equal((await send("route", routeRequest, await sign({ pfdid: "wrong" }))).status, 401);
    assert.equal((await send("route", { ...routeRequest, epoch: 1 })).status, 400);
    assert.equal((await send("route", { ...routeRequest, token: first.token })).status, 400);
    const routingResponse = await send("route", routeRequest);
    assert.equal(routingResponse.headers.get("Cache-Control"), "no-store");
    const initialRoute = await routingResponse.json();
    assert.deepEqual(Object.keys(initialRoute).sort(), ["epoch", "holder", "remainingSeconds", "routingId"]);
    assert.equal(initialRoute.holder, "host");
    assert.equal(initialRoute.epoch, 1);
    assert.equal((await send("renew", { ...fence(first), session: first.routingId })).status, 409);
    ...
    assert.equal(winner.routingId, first.routingId);
    const migratedRoute = await (await send("route", routeRequest)).json();
    assert.equal(migratedRoute.holder, winner.holder);
    assert.equal(migratedRoute.epoch, 2);
    assert.ok(migratedRoute.remainingSeconds > 0);
    assert.equal((await (await send("read", initial)).json()).token, winner.token, "routing reads cannot rotate or change a fence");
    ...
    assert.equal((await (await send("route", routeRequest)).json()).remainingSeconds, 0);
