...
public sealed class LobbyReplica
{
    ...
    public bool Accept(LobbySnapshot state, ulong playerId, ulong sender, ulong server)
    {
        if (server == 0 || sender != server || !state.Players.Any(player => player.Id == playerId) ||
            (State is not null && (state.Session != State.Session || playerId != PlayerId || state.AuthorityEpoch != State.AuthorityEpoch || state.CurrentHostId != State.CurrentHostId || state.Revision <= State.Revision || state.Match < State.Match || (State.Phase == SessionPhase.Arena && state.Phase == SessionPhase.Arena && state.Match == State.Match && state.Map != State.Map))))
        {
            return false;
        }
