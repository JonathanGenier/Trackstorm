...
public static class MigrationCheckpointCodec
{
    ...
    {
        if (bytes.Length is < 36 or > MaximumBytes || bytes[0] != 'T' || bytes[1] != 'C' || bytes[2] != 1 ||
        ...
        try
        {
            ...
            if (wire.Lobby is null || wire.Subjects is null || wire.Deadlines is null)
            {
                throw new ArgumentException("Incomplete checkpoint.");
            }

            var lobby = new LobbyRestoreState(LobbyCodec.DecodeState(wire.Lobby).State, wire.Tick, wire.NextId, wire.Subjects, wire.Deadlines);
    ...
    private sealed record Wire(ulong Sequence, byte[] Lobby, ulong Tick, ulong NextId, Dictionary<ulong, string> Subjects, Dictionary<ulong, ulong> Deadlines, byte[]? Arena, HostRestoreState? Host);
}
