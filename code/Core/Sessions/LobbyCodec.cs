...
public static class LobbyCodec
{
    ...
    public static byte[] EncodeResume(ulong session, ulong player, ulong generation, ulong authorityEpoch = 1) => Pack(1, JsonSerializer.SerializeToUtf8Bytes(new { Command = LobbyCommand.Resume, Session = session, Match = session, Phase = SessionPhase.Lobby, Ready = false, Name = "Player", Player = player, Generation = generation, AuthorityEpoch = authorityEpoch }));
