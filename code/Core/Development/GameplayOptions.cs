...
public static class GameplayOptions
{
    ...
    [
        ...
        new("items.machine_gun_capacity", "Machine Gun", "Capacity", true, c => c.Items.MachineGunCapacity, (c, v) => c with { Items = c.Items with { MachineGunCapacity = checked((int)v) } }),
        new("items.machine_gun_fire_rate", "Machine Gun", "FireRate", false, c => c.Items.MachineGunFireRate, (c, v) => c with { Items = c.Items with { MachineGunFireRate = v } }),
        new("items.machine_gun_range", "Machine Gun", "Range", false, c => c.Items.MachineGunRange, (c, v) => c with { Items = c.Items with { MachineGunRange = checked((float)v) } }),
        new("items.machine_gun_damage", "Machine Gun", "Damage", false, c => c.Items.MachineGunDamage, (c, v) => c with { Items = c.Items with { MachineGunDamage = checked((float)v) } }),
        new("items.machine_gun_falloff_start", "Machine Gun", "FalloffStart", false, c => c.Items.MachineGunFalloffStart, (c, v) => c with { Items = c.Items with { MachineGunFalloffStart = checked((float)v) } }),
        new("items.machine_gun_falloff", "Machine Gun", "Falloff", false, c => c.Items.MachineGunFalloff, (c, v) => c with { Items = c.Items with { MachineGunFalloff = checked((float)v) } }),
        new("items.machine_gun_spread", "Machine Gun", "Spread", false, c => c.Items.MachineGunSpread, (c, v) => c with { Items = c.Items with { MachineGunSpread = checked((float)v) } }),
        new("items.machine_gun_knockback", "Machine Gun", "Knockback", false, c => c.Items.MachineGunKnockback, (c, v) => c with { Items = c.Items with { MachineGunKnockback = checked((float)v) } }),
        new("items.machine_gun_tracer_every", "Machine Gun", "TracerEvery", true, c => c.Items.MachineGunTracerEvery, (c, v) => c with { Items = c.Items with { MachineGunTracerEvery = checked((int)v) } }),
        ...
        new("items.nitro_consumption_per_second", "Items", "Nitro charge consumed (%/s)", false, c => c.Items.NitroConsumptionPerSecond, (c, v) => c with { Items = c.Items with { NitroConsumptionPerSecond = v } }, doublePrecision: true),
        new("vehicle.overspeed_deceleration", "Vehicle", "Overspeed recovery (m/s²)", false, c => c.Vehicle.OverspeedDeceleration, (c, v) => c with { Vehicle = c.Vehicle with { OverspeedDeceleration = checked((float)v) } }),
        new("items.nitro_forward_thrust", "Items", "Nitro forward thrust (N)", false, c => c.Items.NitroForwardThrust, (c, v) => c with { Items = c.Items with { NitroForwardThrust = checked((float)v) } }),
        new("items.nitro_airborne_thrust_scale", "Items", "Nitro airborne thrust fraction", false, c => c.Items.NitroAirborneThrustScale, (c, v) => c with { Items = c.Items with { NitroAirborneThrustScale = checked((float)v) } }),
        ...
        new("items.salvo_count", "Salvo", "Shots per pickup", true, c => c.Items.SalvoCount, (c, v) => c with { Items = c.Items with { SalvoCount = checked((int)v) } }),
        new("items.salvo_interval_ticks", "Salvo", "Shot cooldown (ticks; 30 = 0.5 seconds)", true, c => c.Items.SalvoIntervalTicks, (c, v) => c with { Items = c.Items with { SalvoIntervalTicks = checked((int)v) } }),
        new("items.salvo_range", "Salvo", "Fixed forward range (m)", false, c => c.Items.SalvoRange, (c, v) => c with { Items = c.Items with { SalvoRange = checked((float)v) } }),
        new("items.salvo_launch_height", "Salvo", "Height above vehicle (m)", false, c => c.Items.SalvoLaunchHeight, (c, v) => c with { Items = c.Items with { SalvoLaunchHeight = checked((float)v) } }),
        new("items.salvo_arc_height", "Salvo", "Parabola height above chord (m)", false, c => c.Items.SalvoArcHeight, (c, v) => c with { Items = c.Items with { SalvoArcHeight = checked((float)v) } }),
        new("items.salvo_speed", "Salvo", "Mean flight speed (m/s)", false, c => c.Items.SalvoSpeed, (c, v) => c with { Items = c.Items with { SalvoSpeed = checked((float)v) } }),
        new("items.salvo_blast_radius", "Salvo", "Blast radius (m)", false, c => c.Items.SalvoBlastRadius, (c, v) => c with { Items = c.Items with { SalvoBlastRadius = checked((float)v) } }),
        new("items.salvo_damage", "Salvo", "Maximum damage per round", false, c => c.Items.SalvoDamage, (c, v) => c with { Items = c.Items with { SalvoDamage = checked((float)v) } }),
        new("items.salvo_falloff", "Salvo", "Damage and impulse falloff exponent", false, c => c.Items.SalvoFalloff, (c, v) => c with { Items = c.Items with { SalvoFalloff = checked((float)v) } }),
        new("items.salvo_impulse", "Salvo", "Maximum impulse (N s)", false, c => c.Items.SalvoImpulse, (c, v) => c with { Items = c.Items with { SalvoImpulse = checked((float)v) } }),
        new("items.salvo_marker_scale", "Salvo", "Marker radius relative to blast", false, c => c.Items.SalvoMarkerScale, (c, v) => c with { Items = c.Items with { SalvoMarkerScale = checked((float)v) } }),
        new("items.salvo_marker_width", "Salvo", "Marker ring width (m)", false, c => c.Items.SalvoMarkerWidth, (c, v) => c with { Items = c.Items with { SalvoMarkerWidth = checked((float)v) } }),
        ...
        new("match.kill_target", "Match", "KillTarget (First to Target only)", true, c => c.Match.KillTarget, (c, v) => c with { Match = c.Match with { KillTarget = checked((int)v) } }),
        new("match.duration_ticks", "Match", "Circus duration (ticks; 36000 = 10 minutes)", true, c => c.Match.DurationTicks, (c, v) => c with { Match = c.Match with { DurationTicks = checked((ulong)v) } }),
