...
/// <summary>Explicit version-six movement snapshot encoding, independent of engine and CLR layouts.</summary>
public static class VehicleStateCodec
{
    ...
    public const int SerializedSize = 123;
    ...
    /// <returns>Exactly one version-six snapshot.</returns>
    ...
    {
        if (bytes.Length != SerializedSize || bytes[0] != 6 || (bytes[61] & ~3) != 0)
        {
            throw new ArgumentException("Expected one version-six movement snapshot.", nameof(bytes));
        }
