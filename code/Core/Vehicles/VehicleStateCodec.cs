...
/// <summary>Explicit version-four movement snapshot encoding, independent of engine and CLR layouts.</summary>
public static class VehicleStateCodec
{
    ...
    public const int SerializedSize = 109;
    ...
    /// <returns>Exactly one version-four snapshot.</returns>
    ...
    {
        if (bytes.Length != SerializedSize || bytes[0] != 4 || (bytes[61] & ~3) != 0)
        {
            throw new ArgumentException("Expected one version-four movement snapshot.", nameof(bytes));
        }
