...
internal sealed class GameVersionTests
{
    ...
    {
        Assert.That(GameVersion.Current.ToString(), Is.EqualTo("0.1.1"));
        Assert.That(LobbyCodec.DecodeCommand(LobbyCodec.EncodeCommand(LobbyCommand.Join, null)).GameVersion, Is.EqualTo("0.1.1"));
        Assert.That(LobbyCodec.DecodeCommand(LobbyCodec.EncodeResume(100, 2, 1)).GameVersion, Is.EqualTo("0.1.1"));
        var host = new LobbyAuthority(100, "Host");
        foreach (string incompatible in new[] { "0.1.0", "0.1.2", "0.2.0", "0.1.1.0" })
        {
        ...
        Assert.That(host.Join(10, "0.1.1", "Guest"), Is.EqualTo(2));
    }
    ...
    public void AdmissionWireCarriesVersionAndMissingVersionFailsClosed()
    {
        ...
        byte[] legacy = [(byte)'T', (byte)'L', 7, 1, .. Encoding.UTF8.GetBytes("{\"Command\":0,\"Session\":0,\"Match\":0,\"Phase\":0,\"Ready\":false,\"Name\":\"Guest\",\"Player\":0,\"Generation\":0,\"AuthorityEpoch\":1}")];
        ...
        Assert.That(LobbyCodec.IsReservation(rejection), Is.False);
        Assert.That(LobbyCodec.DecodeVersionMismatch(rejection), Is.EqualTo(version.ToString()));
        byte[] reservation = LobbyCodec.EncodeReservation(100, 2, 1, 1, ReservationResult.Available);
        Assert.That(LobbyCodec.IsReservation(reservation), Is.True);
        Assert.That(LobbyCodec.IsVersionMismatch(reservation), Is.False);
    }
}
