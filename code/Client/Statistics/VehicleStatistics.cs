...
internal static class VehicleStatistics
{
    ...
    internal static IReadOnlyList<StatisticSection> Capture(VehicleSnapshot? state, IReadOnlyList<ItemSlot>? slots, ulong tick)
    {
        ...
        string combat = $"Machine gun rounds: {slot?.Ammo?.Remaining ?? 0}/{slot?.Ammo?.Capacity ?? 0} · Slot 2: {slot?.SecondAmmo?.Remaining ?? 0}/{slot?.SecondAmmo?.Capacity ?? 0}\nHeld item: {item} · Can interact: {state.CanInteract}\nOil traction remaining: {movement.OilTicks / 60f:0.00} s\nNitro charge: {slot?.NitroCharge ?? 0:0.0}% / {slot?.SecondNitroCharge ?? 0:0.0}% · Boost active: {movement.Nitro.Active} · Recovering: {movement.Nitro.Recovering} · Thrust {movement.Nitro.ForwardThrust:0} N · Air ×{movement.Nitro.AirborneThrustScale:0.0} · Speed ×{movement.Nitro.SpeedMultiplier:0.0}\n" +
