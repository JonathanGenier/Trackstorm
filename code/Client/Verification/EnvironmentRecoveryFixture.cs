...
internal static class EnvironmentRecoveryFixture
{
    internal static void Seed(NetworkVehicleArena arena)
    {
        ...
        rocks[1] = new(arena.MapConfiguration.Environment!.FinalStage(1), 0, 0, new(1, 0, 0), default);
    ...
    internal static void Verify(NetworkVehicleArena arena)
    {
        ...
        if (state is null || state.Rocks[0].Stage != 2 || state.Rocks[0].Damage != 17 || state.Rocks[1].Stage != arena.MapConfiguration.Environment!.FinalStage(1) || state.Rocks[1].Offset.X != 1 || !state.Plants[0] || !state.Plants[1])
        {
