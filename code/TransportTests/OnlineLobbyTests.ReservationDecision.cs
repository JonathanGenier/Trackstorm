...
internal sealed partial class OnlineLobbyTests
{
    ...
    public void FreshAdmissionWaitsForPendingStartupLookup(bool host)
    {
        ...
        var store = new ResumeLocatorStore(Path.Combine(Path.GetTempPath(), "trackstorm-pending-startup-" + Guid.NewGuid().ToString("N") + ".json"));
        var hint = new ResumeLocator("ended", 999, 2, 1, User(2).Value, 1, User(1).Value);
        store.Save(hint);
        try
        {
            ...
            if (host)
            {
                client.Create("Fresh lobby", LobbyAccess.Public, null);
            }
            else
            {
