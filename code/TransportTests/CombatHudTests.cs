...
internal sealed class CombatHudTests
{
    ...
    public void SnapshotChangesAndItemMappingAreReadOnly()
    {
        ...
        foreach (HeldItem item in Enum.GetValues<HeldItem>())
        {
            ...
            Assert.That(view.ItemName, Is.EqualTo(item == HeldItem.None ? "EMPTY" : item == HeldItem.Nitro ? "NITRO 100%" : item == HeldItem.ProxyMine ? "PROXY MINE" : item == HeldItem.Salvo ? "SALVO 5" : item.ToString().ToUpperInvariant()));
