...
[Cloudflare authority fencing](docs/features/authority-leases.md) coordinates one short lease per session while gameplay remains P2P. The service owner deploys through the [GitHub-connected dashboard workflow](docs/authority-lease-service.md), and the deployed HTTPS endpoint is bundled for ordinary developers and players without Cloudflare account or Node/npm/Wrangler setup. Worker reachability is verified; live EOS lease behavior and physical-PC migration acceptance remain pending.
