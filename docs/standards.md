...
- Client may reference Core; Core must never reference Client.
...
- Confirm the `Client -> Core` dependency direction and absence of a Shared layer.
- Run formatting verification, restore, build with warnings as errors, and the affected Core tests.
- Run the minimal Godot project when project settings, scenes, or Client integration change.
- Search for dead paths, stale identifiers, and unrelated changes before review.
