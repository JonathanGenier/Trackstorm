...
**VERIFIED:** the Cloudflare deployment completed with the existing `LEASE_SESSIONS` binding, the four public EOS runtime identifiers are configured, the backend deployment tests passed, `GET /health` returned `{"status":"up"}`, and the deployed HTTPS endpoint is bundled in `config/authority-lease-endpoint.json`. Ordinary developers and players require no Node/npm/Wrangler or per-PC endpoint configuration. See [dashboard deployment](../authority-lease-service.md).

**UNVERIFIED:** live EOS Connect token acceptance, lease creation/renewal/takeover, migration through the deployed Worker and real EOS/physical-PC acceptance. `/health` proves only that the Worker is reachable.

First run normal Public lobby host/join, then Public two-PC lobby abrupt host kill and Public two-PC active-match abrupt host kill. Then verify permanent EOS retirement while Cloudflare remains reachable, healthy-client zero periodic reads, former-host CLIENT return after 30 seconds and two minutes, Locked return, outage, P2P-only partition, sequential epochs and 3+ agreement. The latest older-build physical results remain lobby/arena process-kill failures until rerun; local tests and `/health` do not replace them. PR #27 must remain unmerged.
