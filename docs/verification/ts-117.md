...
- Independent delivery review synchronized the Story result conceptually against current `main` after TS-100 merged. The integration preserves TS-100's 20-pickup oval/map preparation behavior while retaining TS-117 Oil behavior, and recalculates the Story version to `0.1.31` / `0.1.31.0`.
- No new local runtime or comprehensive test result is claimed for that delivery sync. Current PR CI remains the independent post-sync machine gate.
