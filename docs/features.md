...
`Core.Vehicles` owns validated immutable tuning, movement decisions, support/air/drift/boost state, plain numeric transform/velocity snapshots, HP, damage math, attribution and destruction. `VehicleMovement.Step` accepts the next `InputFrame`, a collision-resolved `VehiclePhysicsState` observation, and a unit support normal or zero. It advances exactly one configured fixed tick. Restore includes every movement timer; no wall-clock time or engine object enters Core.

Client `VehicleBody` is a Godot `RigidBody3D` adapter. Its custom integrator disables native gravity/force damping and applies Core velocity commands while retaining native collision solving, continuous collision detection, and interactions with other bodies. Contact observations and a short downward support ray cross the boundary as plain numeric inputs. The ray bridges tiny solver separation gaps; it does not preserve grounding during an upward launch. Client validates that the native and Core tick rates agree. Native collision shapes, masses, contact queries, transforms and impulse application remain Client responsibilities. The current arena schedules both bodies at 60 Hz; camera smoothing, flashes and blast visuals use presentation time only.

The authoritative movement snapshot is explicitly **pre-solver**: it contains the latest observed pose and the velocities commanded for the next native collision solve, plus tick, support, drift and boost memory. Its speed is the commanded velocity magnitude, including vertical motion. Native simulation observes the solved result on the next tick. A future authority driver must restore the body/environment at the matching phase along with Core state; input alone cannot reconstruct external collisions.

### Health, Collision Damage and Combat Hooks

`VehicleHealth` is the sole HP mutation owner. Default MaxHP is 100. `ApplyDamage` accepts finite damage, source category, stable instigator identity, bounded context and an ordered authority tick. Negative/zero requests cannot heal or overwrite attribution. Actual damage clamps HP to zero; repair clamps living vehicles to MaxHP. Reaching zero emits exactly one destroyed/death-candidate outcome. Further damage cannot duplicate it, and repair cannot revive a wreck: `Reset` explicitly starts a new life and clears its event/cooldown memory. Destroyed bodies remain physical but Core disables acceleration, steering, drift and boost.

Collision severity is the larger of relative normal closing speed and contact impulse divided by the receiving vehicle's mass. This accommodates native observations made after velocity resolution. Each vehicle evaluates its own received contact, with the other vehicle's stable ID for attribution; walls/props use world identity zero. Core computes zero damage at or below the default 4 m/s threshold, then 3 HP per excess m/s, capped at 100 HP. Client submits the strongest reported contact per body tick. A Core-owned 12-tick cooldown prevents manifold duplicates and sustained contact storms. This cooldown is per victim, so separate strong impacts within that window intentionally coalesce. Low-speed brushes and ordinary support impulses stay harmless.

`DamageEffect` carries nonnegative requested damage, a world-space impulse and a world-space application offset. It is independent of missiles, inventory and item classes. The pure explosion helper linearly fades damage and impulse to zero at the radius, biases the outward direction upward, and uses up as the defined direction exactly at the center. Client queues effects for the next native integration callback, applies damage through Core, and applies the impulse at the supplied offset; native inertia creates rotational response. The arena demonstration uses an 8-metre radius, 55 HP center damage and 15,000 N·s center impulse. Landing or secondary impacts can cause additional collision damage.
...
`VehicleStateCodec` uses an explicit 70-byte version-one little-endian layout: version, tick, thirteen IEEE floats for position/orientation/linear/angular velocity, support/drift flags, and two integer timers. It validates length/version/reserved flags, finite physics, unit orientation and state invariants. `VehicleDamageState`, `DamageEvent` and `DamageContext` have validated JSON-serializable scalar contracts. Health snapshots include the last event and collision cooldown tick, so restoring them does not repeat death transitions or lose contact-gate memory. Event sequence and ticks are scoped to an explicitly reset vehicle life; a future replication envelope must also identify that life and vehicle.
