...
The pinned SDK caps native packets at 1,170 bytes. A 29-byte frame carries message type, connection nonces, sequence, total length and fragment index. Payloads up to 64 KiB are fragmented into at most 58 packets. Each peer has one fixed 64 KiB assembly buffer per delivery stream. Unreliable fragments may arrive out of order; a newer message replaces an incomplete older assembly, while duplicates and stale sequences are ignored. Reliable replacement of an incomplete message is a protocol failure. Malformed sizes and fragment indices are rejected before copying. No arbitrary-sized reassembly allocation occurs.
