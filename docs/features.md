...
`MatchReceived` exposes each accepted revision once for future leaderboard and result UI, including its score deltas. The arena displays the authoritative waiting/countdown/active/finished state, kills/deaths and winner. Its countdown display uses the host tick and never changes match phase locally. The gameplay protocol generation and EOS compatibility bucket require matching builds so older clients cannot silently join without scoring support.
