...
Development launches pass `-- --transport-host=0.0.0.0:27020` or `-- --transport-connect=127.0.0.1:27020` to Godot. IPv6 uses brackets, for example `[::1]:27020`. The options are mutually exclusive. The diagnostics HUD displays the first active peer's sampled ping when enabled. Without an option, native transport stays uninitialized and ping remains unavailable. The arena remains locally simulated; connecting does not automatically replicate vehicles or input.
