...
Development launches pass `-- --transport-host=0.0.0.0:27020` or `-- --transport-connect=127.0.0.1:27020` to Godot. IPv6 uses brackets, for example `[::1]:27020`. The options are mutually exclusive. The diagnostics HUD displays the first available sampled ping among connected peers when enabled, or unavailable when none has a sample. Without an option, native transport stays uninitialized and ping remains unavailable. These options select the network vehicle arena and replicate player inputs and authoritative vehicle snapshots. Launching without either option retains the local rigid-body arena.
...
Prediction runs immediately after local capture, before waiting for an acknowledgement. A new authoritative snapshot replaces the local simulation boundary, removes only confirmed inputs, and replays the remaining commands in original sequence order. History is bounded to 120 commands (two seconds at 60 Hz). Exhaustion stops the session with an explicit reconnect diagnostic rather than silently dropping required replay history. Serial comparisons accept uint wrap and reject equal, stale and half-range-ambiguous values. Core simulation ticks remain checked ulong values and never wrap silently.
