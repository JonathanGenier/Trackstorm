...
The startup pointer fixture explicitly sizes its Window to 1280×720 before composition, including under the headless display. Pointer clicks require visible, nonempty target bounds fully inside the viewport. Repeated Play → Direct-IP → Back cycles use mouse motion and press/release events through Godot hit testing, wait for newly shown container layout, and assert exactly one Back activation plus immediate navigation/visibility/reset outcomes. This fixture setup does not change production window defaults or navigation.
