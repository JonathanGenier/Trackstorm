...
The startup pointer fixture explicitly sizes its Window to 1280×720 before composition, including under the headless display. Pointer clicks require visible, nonempty target bounds fully inside the viewport. Repeated Play → Back cycles use mouse motion and press/release events through Godot hit testing, wait for the outgoing hoist and incoming drop/settle, and assert input gating plus restored navigation/visibility. This fixture setup does not change production window defaults or navigation.
