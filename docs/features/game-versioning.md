...
Retained-match inspection and abandonment carry the runtime version over the same authenticated lobby stream. The host rejects incompatible control requests before querying or releasing a reservation. Version rejection and reservation responses have distinct packet kinds; the menu preserves the canonical local/host mismatch message and clears its obsolete local routing hint without releasing the host reservation.
