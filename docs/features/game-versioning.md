...
The Windows `application/file_version` and `application/product_version` fields in tracked `export_presets.cfg` are synchronized representations of the canonical source, not independent inputs. `sync-story-version.ps1` updates them to `MAJOR.RELEASE.PR.0` whenever it establishes or recalculates a Story version, while preserving unrelated export preferences. `check-version.ps1` and CI reject missing, duplicated, malformed or drifted fields.
