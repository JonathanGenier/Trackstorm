...
The shell fills the viewport with a black/dark-gray panel, a fixed header containing readable tab buttons, a persistent bottom-right Close button, and one visible content surface. Configs keeps Force Start and search above its internal scroll, with Reset at bottom-left and Apply Settings / Cancel alongside Close. These configuration actions are hidden on Stats and Logs. Stats and Logs keep their existing internal controls. The in-panel close decision temporarily replaces navigation and content, confines focus to Apply / Discard / Stay, and blocks tab shortcuts until resolved.
