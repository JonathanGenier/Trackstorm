...
Entering Play only displays existing coordinator state; it never calls ResumeRetained. A saved routing hint, matching lobby metadata or failed lookup does not prove a resumable reservation. Normal browsing, Host and Back remain available, with the existing explicit Check previous session action for validation. Passive startup lookup cannot replace Main Menu. Explicit validation hides browser controls while checking; only an authority-confirmed reservation presents Reconnect to previous game? Yes / No. Yes calls DecideRetained(true); No calls DecideRetained(false), and browsing returns only after authoritative abandonment and cleanup clear the retained state. A failed lookup or validation restores normal browsing with the existing status and explicit retry action, preserving the hint without claiming abandonment. Networking, reservation and cleanup logic remain in their existing owners.
