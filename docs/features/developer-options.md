...
Salvo adds `items.salvo_*` controls in the Salvo category through the same catalog, persistence and configuration boundary. See [arcing Salvo](items.md#arcing-salvo) for defaults, bounds and live-edit semantics. Marker parameters are shared tuning; marker nodes remain local presentation and never replicated entities.
