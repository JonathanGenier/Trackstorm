...
Authenticated `POST /lease/{operation}` accepts exactly `{session, epoch, token}` and returns `{session, holder, epoch, token, remainingSeconds, routingId}`. The Worker derives holder identity only from the verified JWT subject. Its private Durable Object binding receives that identity; public request bodies cannot override it.

`routingId` is an opaque read-only address of that same Durable Object, not the private session key or a fencing token. Authenticated `POST /lease/route` accepts `{session: routingId, epoch: 0, token: ""}` and returns only `{routingId, holder, epoch, remainingSeconds}` from the existing durable lease record. The adapter treats the address as provider-neutral opaque data; only the Worker interprets it as an object ID. No routing registry, second authority record, gameplay state or additional service is introduced. Routing reads cannot create, renew, release or take over a lease, and never expose the private session key or token. Write routes still address objects by the private session name, so an object ID cannot substitute for that key.

Arena resume locators may persist this read-only address. After EOS membership recovery, restart uses it to resolve the current fenced host independently of stale `gameHost`/`epoch` metadata or EOS ownership. The coordinator rejects expired, malformed, mismatched, regressed, conflicting and self routes; responses taking three seconds or longer are discarded and failed lookup retries remain bounded in frequency. This selects a connection target only: authenticated P2P plus Core Resume still validate session, subject, retained PlayerId and generation. Reads never grant simulation permission. Healthy connected clients still send no periodic lease reads.
...
The service lease is **10 seconds** and current-host renewal **2 seconds**. Healthy connected clients make no periodic lease reads. The existing migration state activates trusted observation on host/P2P loss, missing checkpoints, authority pause or survivor agreement; observers then read at most once per **1 second**, with an immediate read when observation starts. Observation stops on recovery, completed client installation or terminal failure. Each new observation period requires a fresh service response; cached expiry cannot authorize takeover. Only the current authority renews continuously. Local gameplay permission remains **8 seconds from request start on a monotonic clock**. Responses taking three seconds or more are discarded, and a retired local epoch cannot be revived by a delayed successful renewal. Network and storage delay consume permission rather than extending it.
...
Permanent EOS authority retirement uses the coordinator's existing proof-expiry or explicit local-retirement boundary. Before the next lease pump, migration freezes gameplay, irrevocably retires that epoch in the lease client and sends the existing frozen-host departure control. The pump drains any in-flight response without restoring authority, then attempts conditional release with the latest valid token. A discarded response, rejected release or service outage leaves expiry as the safe fallback; no further renewals or idle reads are sent by the retired host. Temporary proof delays inside the existing validity window and ordinary client/P2P interruptions do not retire or release a healthy host. Intentional Leave uses the same freeze-before-release ordering.
...
Storage/routing/authentication failure grants no authority. An existing host stops at its local deadline, and a survivor needs a fresh service observation and successful conditional write. Lost acknowledgements can lengthen exclusion without granting permission. Renewal token changes provide evidence of continuing old-host progress for checkpoint freshness. A first observation without a prior token conservatively uses its request timestamp too: the observer cannot rule out missed renewal during a service outage. This can refuse recovery after a prolonged observation outage even when the first response reports expiry, rather than restoring an unsafe old checkpoint.
...
The deployed public URL in [bundled endpoint configuration](../../config/authority-lease-endpoint.json) is embedded in the Client assembly for both editor and export. Gamers require no environment variables, Cloudflare account or external tooling. `TRACKSTORM_LEASE_URL` is an optional explicit developer/test override; invalid overrides fail instead of silently falling back. The deployed Worker's `/health` response verifies reachability, not live EOS Connect authentication, lease acquisition/renewal, migration or physical-PC acceptance.
