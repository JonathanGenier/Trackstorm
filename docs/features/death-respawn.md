...
`RespawnConfiguration.ClearHeldItemOnDeath` defaults to true. `ItemAuthority` clears both held slots and their selection in the same committed death batch. With retention explicitly enabled, both held items remain unavailable during death/waiting and transfer to the new life with fresh individual grant tokens on respawn; the selected index is retained and its life-scoped command watermark resets. Pending uses clear each step; an old life/token cannot spend the retained or replacement item. Final departure and explicit development resets discard inventory; authenticated reconnect grace retains current authoritative inventory.
