...
Development launches pass `-- --transport-host=0.0.0.0:27020` or `-- --transport-connect=127.0.0.1:27020` to Godot. IPv6 uses brackets, for example `[::1]:27020`. The options are mutually exclusive. The diagnostics HUD uses `TransportDiagnostics` to combine the current connection lifecycle and optional local quality with the local host-published `PlayerLatency` entry. Its RTT therefore matches the leaderboard exactly, using the same formatter and publication freshness rules, without native SDK types or provider branches. Missing publications never fall back to a separate client-side RTT sample. Hosts have no upstream ping and show unavailable. Initial admission, reconnect/checkpoint recovery and disconnected states suppress numeric latency. The projection has no sample cache: gateway/session replacement and fresh peer IDs cannot reuse prior display values. GNS statistics are queried only for a currently connected handle; EOS owns nonce-bound expiring probes as described in [EOS P2P](eos-p2p.md). The neutral projection also preserves the adapter-provided name and nullable quality fractions for later consumers; it adds no Dev Mode controls. These options open the development lobby in host or join mode; `--player-name=Name` supplies the requested display name. Without either option the main scene opens Main Menu, whose Browse action opens the EOS multiplayer browser inside MenuShell; Direct-IP Host/Join is an explicit developer fallback, initializing transport only when requested. `-- --local-practice` selects the local rigid-body arena. Lobby gameplay starts only after the host validates a Start Match intent.
...
## Large reliable boundaries

Client `ReliableMessageGateway` shares one transfer owner among the lobby, vehicle and migration drivers using the same gateway. Small reliable messages and unreliable traffic keep their native path. Large boundaries first use bounded Brotli compression when it saves space; exact decompressed size and complete input consumption are validated before delivery. Any remaining large reliable state is framed into 60,000-byte chunks (20-byte header), below both adapters' 64 KiB payload ceiling. Each peer has at most two unacknowledged chunks. Later reliable messages wait behind that boundary; unrelated unreliable traffic can continue. The native EOS nonce/fragmentation and Direct-IP reliability contracts remain unchanged.

Assembly requires the exact message ID, declared size, next offset and reliable delivery. Only a complete payload reaches existing session/epoch/generation/revision and gameplay validation; partial state cannot install. Replayed completed IDs do not publish again. Malformed chunks disconnect the offending peer. Thirty seconds without transfer progress fails closed; disconnect/Stop clears partial state. A successor/rebound connection receives a new complete checkpoint through the existing recovery path.

Message size is bounded by the 96 MiB migration envelope plus three bytes of framing; item payloads allow 64 MiB and resume adds 64 KiB for the existing nested records. These ceilings cover the lifetime-derived Oil allocation bound and migration JSON expansion. Per-gateway send backlog and in-progress receive assembly each have a 128 MiB budget; at most 256 reliable messages wait. Exhaustion disconnects rather than installing partial state or silently discarding Oil. The bounds are resource protection, not promises of real-time performance at extreme tuning. Normal untouched Oil state uses absolute expiry ticks. Ordered item publications reference the preceding publication revision when patches, consumed counts and overlap latches are unchanged, avoiding retransmission during unrelated projectile or cooldown updates. Changed Oil, joins, resume and migration carry complete state; a missing or wrong baseline is rejected.

Chunking does not change authority or trust: complete message consumers still validate the current host, session, epoch, generation, schema and revision. Local UDP and fake-provider EOS tests do not establish separate-device Internet throughput.