...
Accepting a larger checkpoint retires older two-player copies, and a currently larger roster cannot select a two-player checkpoint. The service retirement cohort check additionally prevents selection from an obsolete roster after membership changes. If the provider cohort and retained checkpoint disagree, recovery fails closed.

## Complete checkpoint

The version-two `TC` migration checkpoint composes the existing complete version-two `TR` resume codecs with authority continuation:
...
- The **monotonic checkpoint receipt freshness bound** requires the selected external checkpoint to have been received no more than four monotonic seconds before the trustworthy old-authority retirement boundary. Each retained copy stores its local monotonic receipt timestamp outside the serialized checkpoint. Intentional Leave uses the reliably received frozen-host departure boundary; unexpected loss uses the original validated EOS service-departure event timestamp, after the existing lease/cohort checks establish that event as safe. UTC and wall-clock time are not used.

Required fencing or agreement time after the retirement event does not enlarge rollback because the old authority is already retired. A checkpoint received after that boundary is ineligible, and an equal/older sequence cannot replace a retained timestamp, so delayed or replayed packets cannot make old state fresh. The service membership lease prevents authority from progressing indefinitely after losing online membership. Healthy hosts can continue without reachable checkpoint recipients; that does not promise a recent recoverable copy for an isolated client. A survivor isolated for longer than the four-second recoverability window before the host retires fails migration even when the healthy host continued normally. Network stalls that prevent a recent common boundary produce intentional fail-closed recovery rather than unbounded rollback; arbitrarily long partitions are not lossless.
...
The former host may resume its reserved stable player identity as an ordinary client, with no Start/Return, tuning, Force Start or Give Item privilege. Developer controls require current, unfrozen authority; Give Item targets the elected host's stable player ID. Epoch changes discard stale editor drafts. Diagnostics expose current host, epoch, configuration revision and bounded migration/checkpoint counters without credentials. The local resume locator also supports the original host identity. Normal host Leave retains a short-lived locator and exposes **Resume previous session**, without automatically undoing Leave. Both this action and process restart recover membership by lobby-ID search and ordinary EOS Join, wait for replacement routing instead of connecting back to self, and send Core Resume rather than fresh Join. Retained Locked players do not re-enter a password; fresh players still require the verifier. Restart reads current lobby host/epoch routing and still requires authenticated subject/generation validation by Core. Reservation expiry remains bounded; return after expiry does not reclaim the old slot.

## Stale traffic and one-shot safety

Lobby protocol version three carries host and epoch; all post-admission commands check the authority fence. The version-two `TG` envelope checks session, authority epoch and recipient connection generation before any nested vehicle, item, match or prop codec/event consumer runs. Native connection nonces, subscription generations, monotonic peer handles and coordinator operation generations reject retired connections and delayed callbacks. The version-two `TP` standings diagnostics also fence authority epochs and use the elected host identity. Online compatibility is `trackstorm-lobby-9`: the service retirement policy and resumable availability metadata must not mix with older acknowledgement-lease hosts.
