...
The [input owner](input.md) observes that suppression and immediately releases gameplay mouse capture when the overlay opens, including Settings and F1 Developer Options. Closing the final menu restores hidden/captured gameplay input. Main Menu, lobby and Podium flows expose a free native pointer. Keyboard/controller focus navigation remains independent of pointer visibility; using the mouse after controller navigation works without leaving the menu. Focus loss releases capture, and focus regain preserves whichever gameplay/menu context is current.
...
The arena has no permanent Settings, End Session/Return or Leave button. Production [Podium](post-match.md) owns the post-match destination controls; ESC can still open this menu above it. Standalone standings fixtures retain their ESC footer. The lobby retains its own Leave control. Game Menu Leave to Main Menu uses `DevelopmentSession.ReturnToMainMenu`, clearing browser intent before the shared Leave cleanup gate. Reliable client departure delivery, transport release, online membership cancellation and saved-resume clearing retain their existing owners. Re-entry constructs a new arena. Local practice releases its arena and uses the normal session factory so the returned Main Menu retains navigation, Settings, Quit and online callbacks.

Game Menu Quit and the window close request share the bootstrap's exit path. They request the same session Leave and wait for session and online membership cleanup, while pumping normal runtime callbacks. An online cleanup failure remains visible with a retryable Quit action. Successful cleanup proceeds through normal tree teardown: settings flush, native transport disposal, EOS platform release and terminal SDK shutdown retain their existing owners. Online host Leave uses the existing host-migration drain and election; direct-IP sessions without authenticated migration close on host departure. The menu introduces no separate migration path.
...
`check-menu.ps1 -GodotPath <Godot .NET executable>` exercises the production bootstrap with isolated settings storage and real local UDP sessions. It covers synchronized solo Countdown, ESC/Back, synthetic keyboard/gamepad navigation, remapping, local suppression with continued authoritative and remote progress, setting updates/persistence, unified Developer Options, retired buttons, viewport bounds, Leave/re-entry and production Quit. `-Visual` also captures all categories and four viewport sizes. `check-settings.ps1 -Visual` separately verifies restart persistence, save failure/retry and display preview/confirmation/reversion. Lobby and standings harnesses retain direct coverage of the underlying Return command without recreating the retired UI. Native synthetic input is not evidence of physical controller ergonomics or separate-PC authenticated EOS behavior.

The menu harness also clicks a native category control after controller navigation and checks logical LMB item presses across menu returns. `-Visual` additionally verifies actual Godot mouse modes through gameplay, ESC, Settings/categories, Main Menu/lobby, diagnostic overlays, leave/re-entry and synthetic focus notifications. Headless mode skips mouse-mode assertions because the dummy display server cannot establish native capture. Physical alt-tab, OS pointer confinement/visibility and controller ergonomics still need real-device validation.
