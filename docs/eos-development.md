...
Use two independent PCs/profiles on the same build for the remaining EOS checks. These reconnect checks keep the host running. Test host loss separately using the migration procedure below.
...
4. Stay offline beyond grace. Confirm the host releases the slot/vehicle once, stale resume fails, and the client returns to a clear failure/menu state. Close the host and confirm either coherent migration with an eligible roster or bounded failure. Explicitly Leave while connected and verify immediate slot release, cleared saved locator and no automatic resume on restart.
...
Online gameplay uses the [Cloudflare authority-fencing service](authority-lease-service.md), with one Durable Object per logical lease session. The release owner bundles its verified HTTPS URL in `config/authority-lease-endpoint.json`; editor and exported players require no environment-variable or Cloudflare setup. The actual URL is pending dashboard deployment, so the current null default fails closed. `TRACKSTORM_LEASE_URL` remains an optional developer/test override. Node/npm/Wrangler are not ordinary Trackstorm prerequisites. Verify the deployed Worker's actual Connect token acceptance before testing: health alone is insufficient, and the host must show a held lease. Membership proof age and trusted fencing status are separate diagnostics. First rerun Public two-PC lobby host kill, then Public two-PC active-match host kill; only after both pass continue with return, Locked, outage, partition and sequential migration. Real authenticated P2P harnesses also use the bundled endpoint or explicit override.
...
1. In a **Public lobby**, abruptly kill the host process. Confirm the sole survivor becomes the replacement in the same session, with unchanged player IDs, cleared Ready, and working Ready/Start instead of timing out to the menu. Measure takeover against the remaining ten-second lease plus service polling/agreement delay, independently of the 30-second reconnect grace. Delayed EOS host-departure notification must not gate promotion.
2. In a **Public active arena**, record HP, held items, active missiles, pickup cooldowns, dead-player respawn deadlines and standings. Abruptly terminate the host process. Confirm freeze/recovery feedback, one next epoch, the same match, recent checkpoint rollback, one vehicle per player, and coherent gameplay after resync without waiting through player reconnect grace. Check no repeated death, kill, pickup or winner effects. Both Public crash cases must pass before continuing to the remaining scenarios.
