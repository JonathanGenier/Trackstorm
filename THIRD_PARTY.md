...
| Frontend media | [Runtime/import notes](assets/frontend/README.md) and [checksummed source/derivative manifest](assets/frontend/sources.json); the TS-85 MOV and MP3 are project-provided assets with no third-party license asserted. The unchanged MOV is retained through Git LFS. Its silent Ogg Theora derivative is required by Godot's built-in video player; the unchanged MP3 is the authoritative frontend music. FFmpeg is an external authoring tool, not a distributed runtime dependency |
