...
| Lease-service authentication | [Service project](code/LeaseService/Trackstorm.LeaseService.csproj) pins `Microsoft.AspNetCore.Authentication.JwtBearer` 10.0.12; restored authentication dependencies are IdentityModel 8.19.2 and Microsoft.Bcl.Cryptography 10.0.2, from NuGet. Microsoft-authored MIT packages; [source revisions, copyright and license notice](docs/licenses/authority-leases.txt) are included in the standalone service publish output. They are not gameplay Client dependencies. The ASP.NET Core 10 runtime is an operator-installed prerequisite with its own notices |
